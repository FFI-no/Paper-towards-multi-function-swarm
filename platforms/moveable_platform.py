...
class MoveablePlatform(Platform):
    def __init__(self, agent, config):
        ...
        self.acceleration = ta.array([0.0,0.0], float)
        self.velocity = ta.array([0.0,0.0], float)
    ...
    def step(self, current_time, case):
        ...
        self.position = np.maximum(ta.array([0.,0.]), np.minimum(ta.array(case.config["config_simulator"]["grid_size"]), self.position))
